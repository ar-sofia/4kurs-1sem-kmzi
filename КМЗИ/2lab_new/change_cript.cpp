...
uint32_t sp_network_32(uint32_t plaintext_block, uint32_t round_key)
{
	...
	// Применяем S-блок к каждым 4 битам (8 раз)
	uint32_t result = 0;
	for (int i = 0; i < 8; ++i) {
	...
	// Применяем P-блок к 32-битному блоку
	const int A = 23;
	const int B = 6;
	const int m = 32;
	uint32_t p_result = 0;
...
uint32_t block_encryption(uint32_t key, uint32_t plain_block)
{
	...
	state = sp_network_32(state, key);

	return state;
}

// Функция шифрования в режиме CBC с дополнением типа 1
vector<uint32_t> encrypt_cbc(uint32_t key, vector<uint32_t> &plaintext, size_t fileSize,
uint32_t iv)
{
	size_t len = fileSize / 4;
	if (fileSize % 4 != 0) {
		len += 1;
	}
	vector<uint32_t> cipher_text;

	uint32_t previous_block = iv; // Начинаем с синхропосылки

	// Шифруем все полные блоки в режиме CBC
	for (size_t i = 0; i < len; i++) {
		// XOR с предыдущим блоком шифртекста (или IV для первого блока)
		uint32_t xored_block = plaintext[i] ^ previous_block;

		// Шифруем блок
		uint32_t encrypted_block = block_encryption(key, xored_block);

		cipher_text.push_back(encrypted_block);
		previous_block = encrypted_block; // Сохраняем для следующего блока
	}
	...
	// Шифруем блок с длиной в режиме CBC
	uint32_t xored_length = length_block ^ previous_block;
	uint32_t encrypted_length = block_encryption(key, xored_length);
	cipher_text.push_back(encrypted_length);
...
int main()
{
	...
	const char *filename = "Var11/Var11/key10.in";
	...
	const char *iv_filename = "Var11/Var11/IV10.in";
	ifstream ivFile(iv_filename, ios::binary);
	if (!ivFile) {
		cerr << "Error: Could not open IV file " << iv_filename << endl;
		return 1;
	}
	...
	ivFile.close();
	iv = (iv >> 24) | ((iv >> 8) & 0x0000FF00) | ((iv << 8) & 0x00FF0000) | (iv << 24);

	cout << "IV: " << hex << iv << " (" << bitset<32>(iv) << ")" << endl;

	// Чтение входных данных - БЕЗ перестановки байтов
	filename = "Var11/Var11/test10.in";
	...
	for (size_t i = 0; i < plaintext.size(); ++i) {
		cout << "Block " << i << ": " << hex << plaintext[i] << " (" << bitset<32>(plaintext[i])
		<< ")" << endl;
	}

	// Шифрование в режиме CBC
	vector<uint32_t> cipher_text = encrypt_cbc(key, plaintext, fileSize, iv);

	// Запись результата с перестановкой байтов
	const char *outputFilename = "out_my/test10.out";
	...
	for (size_t i = 0; i < cipher_text.size(); ++i) {
		cout << "Block " << i << ": " << hex << cipher_text[i] << " (" << bitset<32>(cipher_text[i])
		<< ")" << endl;
	}
