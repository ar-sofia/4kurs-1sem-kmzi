...
uint32_t whitening(uint32_t block, uint32_t key)
{
    return block ^ key;
}
...
uint32_t block_encryption(uint32_t key, uint32_t plain_block)
{
    ...
    // (k1<3)|(k2^19914)
    uint16_t k1_shifted_3 = (k1 << 3) | (k1 >> 13);
    uint16_t round_key3 = k1_shifted_3 | (k2 ^ 19914);
    
    uint16_t round_key4 = k2;
    uint16_t round_key5 = k1 ^ 29867;
    
    // 32-битный ключ для отбеливания: k1^29867,k1
    uint32_t whitening_key = (static_cast<uint32_t>(round_key5) << 16) | k1;

    // Новая последовательность раундов: tftfffw
    cipher_block = tau_permutation(cipher_block); // t
    cipher_block = feistel(cipher_block, round_key1); // f
    cipher_block = tau_permutation(cipher_block); // t
    cipher_block = feistel(cipher_block, round_key2); // f
    cipher_block = feistel(cipher_block, round_key3); // f
    cipher_block = feistel(cipher_block, round_key4); // f
    cipher_block = whitening(cipher_block, whitening_key); // w
...
int main()
{
    const char *filename = "exemple_key.in";
