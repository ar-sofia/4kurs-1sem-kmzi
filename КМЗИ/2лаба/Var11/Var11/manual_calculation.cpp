...
uint32_t sp_network_round(uint32_t input_block, uint32_t round_key)
{
    ...
    cout << "S-box left:  " << hex << left << " (" << bitset<16>(left) << ")" << endl;
    cout << "S-box right: " << hex << right << " (" << bitset<16>(right) << ")" << endl;
    
    state = (static_cast<uint32_t>(left) << 16) | right;
    cout << "After S-box: " << hex << state << " (" << bitset<32>(state) << ")" << endl;
...
int main()
{
    ...
    cout << "Key (after byte swap): " << hex << key << " (" << bitset<32>(key) << ")" << endl << endl;
    ...
    cout << "Input data (" << fileSize << " bytes):" << endl;
    ...
    for (size_t i = 0; i < len; i++) {
        cout << "=== Block " << i << " ===" << endl;
        cipher_text.push_back(sp_network_round(plaintext[i], key));
    }
    ...
    cout << "=== Length Block ===" << endl;
    cipher_text.push_back(sp_network_round(length_block, key));
